...
pacman::p_load(tidyverse, ggplot2, lubridate, gapminder, scales, gganimate, here, ggthemes, rdrobust)
```
...
1. Start with raw data on both sides of the cutoff
2. Bin observations along the running variable
3. Collapse to bin averages to remove noise
4. Zoom in on the bins just around the cutoff
5. The **jump** at the cutoff is the treatment effect
...
3. Estimate regressions within that window
...
1. **Dummy variable approach**: Create dummies for each bin, regress outcome on the dummies ($R^{2}_{r}$). Double the number of bins ($R^{2}_{u}$). F-test: $$F = \frac{R^{2}_{u}-R^{2}_{r}}{1-R^{2}_{u}}\times \frac{n-K-1}{K}$$

2. **Interaction approach**: Include interactions between bin dummies and the running variable, then joint F-test on the interaction terms
