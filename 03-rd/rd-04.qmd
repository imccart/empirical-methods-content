...
modelsummary(rating.ols,
  coef_rename = function(x) gsub("factor\\(Star_Rating\\)", "Star Rating = ", x),
  gof_map = c("nobs", "r.squared"),
  title = "Market Share by Star Rating (OLS)")
...
modelsummary(list("(1)"=star25.1, "(2)"=star25.2, "(3)"=star25.3, "(4)"=star25.4),
...
- OLS on full sample: `r round(est1,3)*100`% increase in market share for 2.5-star versus 2.0-star plans
...
rdrobust_tidy <- function(est, label) {
  tibble(
    Spec = label,
    Estimate = round(est$coef[1], 4),
    `Robust CI` = paste0("[", round(est$ci[3,1], 4), ", ", round(est$ci[3,2], 4), "]"),
    BW = round(est$bws[1,1], 3),
    N = est$N[1] + est$N[2]
  )
}

bind_rows(
  rdrobust_tidy(est1, "Fixed (h=0.125)"),
  rdrobust_tidy(estopt, "Optimal")
) %>%
  knitr::kable()
...
tibble(
  ` ` = c("T-statistic", "P-value"),
  Value = c(round(dens1$test$t_jk, 3), round(dens1$test$p_jk, 4))
) %>%
  knitr::kable()
...
rdrobust_tidy(est225, "2.5 vs 2.0 (h=0.125)") %>%
  knitr::kable()
...
rd_results %>%
  ...
  geom_pointrange(aes(ymin=ci_lower, ymax=ci_upper),
                  color="#012169", size=0.8, linewidth=0.8) +
  coord_flip() +
  theme_bw() +
  labs(x=NULL, y="RD Estimate (Market Share)",
       title="Effect of higher star rating on market share",
       subtitle="Bias-corrected estimates with robust 95% CIs, h = 0.125")
```
